...
import { Vendor, EventBlueprint } from "@/types";
...
export default function VendorCard({ vendor, blueprint }: Props) {
  ...
  const handleGenerateOutreach = async () => {
    ...
    try {
      ...
      setOutreach(data.message ?? "Unable to generate message.");
    } catch {
      setOutreach("Error generating message. Please try again.");
    } finally {
  ...
  return (
    <div className="glass-card p-5 md:p-6 flex flex-col gap-4 hover:border-violet-500/25 transition-all">
      {/* Header */}
      ...
      <div className="flex flex-wrap gap-2">
        {vendor.tags.map((tag) => (
          <span
            key={tag}
            className="px-2.5 py-1 rounded-full text-xs bg-slate-800 text-slate-400 border border-slate-700"
          >
            {tag}
          </span>
        ))}
